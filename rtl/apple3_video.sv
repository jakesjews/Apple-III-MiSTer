...
module apple3_video (
	...
	always_comb begin : address_generation
		...
		character_request = character_write && (v_count == 9'd261) && (h_count >= 10'd700) && (h_count < 10'd764);
		request_index     = line_request ? (h_count[6:0] - 7'd88) : (h_count[6:0] - 7'd60);
		physical          = 19'h00000;
		gfx_line          = 13'd0;
		gfx_page          = 15'd0;
		text_base         = 11'd0;
		...
		if (character_request) begin
			...
			physical  = {4'hf, 15'h0478 + {5'b00000, char_i, 7'b0000000} + {12'b000000000000, char_k}};
		end else if (line_request && (next_y < 9'd192)) begin
			if (!fetch_mode[2]) begin
				...
				gfx_line = {2'b00, (text_line_base(next_y[7:3]) - 11'h400)} + {graphics_row, 10'b0000000000};
				...
				gfx_page = video_mode[2] ? (apple2_hires ? 15'h2000 : 15'h4000) : 15'h0000;
	...
	always_ff @(posedge clk) begin : prefetch_and_character_load
		...
		if (request_valid_d) begin
			if (request_character_d) begin
				...
				character_ram[{ram_q[14:8], request_index_d[4:3], request_index_d[2]}] <= ram_q[7:0];
			end else begin
				if (request_index_d < 7'd40) begin
					line_buffer_low[request_index_d] <= request_lane_d ? ram_q[15:8] : ram_q[7:0];
				end else begin
					line_buffer_high[request_index_d-7'd40] <= request_lane_d ? ram_q[15:8] : ram_q[7:0];
				end
			end
		end
		...
		if (!hblank && !vblank && screen_enable) begin
			case (pixel_mode)
				4'd0, 4'd1: begin
					...
					char_code    = video_mode[2] ? line_buffer_high[h_state] : line_buffer_low[h_state];
					colour_byte  = video_mode[2] ? line_buffer_low[h_state] : line_buffer_high[h_state];
				...
				4'd2, 4'd3: begin
					...
					glyph_column = (state_dot >= 4'd7) ? state_dot[2:0] + 1'b1 : state_dot[2:0];
					glyph = character_ram[{char_code[6:0], glyph_row}];
					pixel_on = glyph[glyph_column];
					invert_pixel = !char_code[7] && (!glyph[7] || flash_count[3]);
					pixel_on = pixel_on ^ invert_pixel;
				...
				4'd8: begin
					...
					colour_byte      = video_mode[2] ? line_buffer_high[byte_position] : line_buffer_low[byte_position];
