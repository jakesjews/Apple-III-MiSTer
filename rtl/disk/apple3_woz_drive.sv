...
module apple3_woz_drive (
	...
	flux_drive drive (
		...
		.RESET              (engine_reset),
		.PHASES             (drive_phases),
		.IMMEDIATE_PHASES   (drive_phases),
		.LATCHED_SENSE_REG  (3'd0),
		.IWM_MODE           (5'd0),
		.MOTOR_ON           (motor_on),
		.SW_MOTOR_ON        (motor_on),
		.DISKREG_SEL        (1'b0),
		.SEL35              (1'b0),
		.DRIVE_SELECT       (1'b0),
		.DRIVE_SLOT         (1'b0),
	...
	woz_floppy_controller #(
		...
		.stable_side         (1'b0),
		.bit_data,
		.bit_data_in         (write_data),
		.bit_we              (bit_we && !write_protect && valid),
		.bit_wr_addr         (write_addr),
		.track_load_complete (load_done),
		.is_flux_track       (is_flux),
		.flux_data_size      (flux_size),
		.flux_total_ticks    (flux_ticks),
		.track_data_valid    (valid),
		.disk_type_mismatch  (mismatch),
		.disk_write_protected(info_wp),
		.optimal_bit_timing  (timing),
		.dbg_load_sum        (),
		.dbg_load_bytes      (),
		.dbg_load_blocks     ()
