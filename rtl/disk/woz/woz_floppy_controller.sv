...
module woz_floppy_controller #(
    ...
    wire [BRAM_ADDR_WIDTH-1:0] save_bram_addr = {blocks_processed[BRAM_ADDR_WIDTH-10:0], sd_buff_addr[8:0]};
...
`endif
    ...
    // WOZ1 writes require preserving its unaligned record metadata; flux tracks
    // require re-encoding timing intervals. Keep those representations read-only.
    assign disk_write_protected = img_readonly || (woz_valid &&
        ((info_wp != 8'd0) || is_woz_v1 || have_flux));
    assign optimal_bit_timing   = woz_valid && !is_woz_v1 ? info_bit_timing : 8'd0;
    ...
    // hps_io supports up to 16 KiB in one transaction. Reading the whole
    // bitstream track avoids a host polling round trip for each 512 bytes.
    // Larger flux tracks use successive bounded bursts; writes stay one block.
    reg [15:0] read_block_base;
    wire [15:0] read_blocks_left = trk_block_count - read_block_base;
    wire [5:0] read_burst_blocks = read_blocks_left > 16'd32 ? 6'd32 : read_blocks_left[5:0];
    assign sd_blk_cnt = state == S_READ_TRACK ? read_burst_blocks - 6'd1 : 6'd0;
    wire [15:0] dma_block = read_block_base + {11'd0, sd_buff_addr[13:9]};
    ...
    always @(posedge clk) begin
        ...
        if (!old_ack && sd_ack && (request_issued || sd_rd || sd_wr)) begin
	                ...
	                S_SCAN_WOZ: begin
	                    ...
	                    // Only count block completion if we had an active transfer (normal parsing)
	                        ...
	                        if (parser_done) begin
                                if (header_ok && info_disk_type == (IS_35_INCH ? 2 : 1))
                                    woz_valid <= 1'b1;
                                else begin
                                    scan_failed <= 1'b1;
                                    woz_valid <= 1'b0;
	                            ...
	                            pending_track_id <= track_id;
	                            load_side <= track_id[0];
	                        end else if (scan_blocks >= SCAN_BLOCK_LIMIT || (({48'd0,scan_blocks} + 1) << 9) >= img_size) begin
	                            woz_valid <= 1'b0;
	                            busy <= 0;
	                            state <= S_INIT;
	                            scan_failed <= 1'b1;
...
`endif
	                ...
	                S_SEEK_LOOKUP: begin
                        ...
                        22: begin
                             ...
                             if (trks_index == 8'hFF) begin
	                             ...
	                             end else if (trks_index >= 8'd160) begin
                                 state <= S_INIT;
                                 scan_failed <= 1;
                                 woz_valid <= 0;
                                 busy <= 0;
                        ...
                        19: begin
                             ...
                             if (!sd_ack) sd_rd <= 1'b1;
                             if (trk_block_count == 0 ||
                                 trk_block_count > (1 << (BRAM_ADDR_WIDTH-9)) ||
                                 ({48'd0,trk_start_block} + {48'd0,trk_block_count}) * 512 > img_size + (is_woz_v1 ? 511 : 0) ||
                                 (!pending_is_flux && !is_woz_v1 && trk_bit_count > 32'd131071) ||
                                 (!is_woz_v1 && (trk_bit_count == 0 ||
                                  trk_bit_count > ({16'd0,trk_block_count} << (pending_is_flux ? 9 : 12))))) begin
                                 state <= S_INIT;
                                 scan_failed <= 1;
                                 woz_valid <= 0;
                                 busy <= 0;
                ...
                S_READ_TRACK: begin
                    ...
                    if (old_ack && !sd_ack && transfer_active) begin
                        ...
                        blocks_processed <= read_block_base + {10'd0,read_burst_blocks};
                        if (read_block_base + {10'd0,read_burst_blocks} >= trk_block_count) begin
                            // Track load complete - store to appropriate RAM
                            $display("WOZ_CTRL: Track %0d load complete (%0d blocks)", pending_track_id, read_block_base + {10'd0,read_burst_blocks});
                            ...
                            // Next block
                            sd_lba <= sd_lba + {26'd0,read_burst_blocks};
                            read_block_base <= read_block_base + {10'd0,read_burst_blocks};
                        end
                    end
                end
...
`endif
            ...
            // This runs in parallel with state machine waiting for sd_ack
	            ...
	            if (old_ack && !sd_ack && state == S_READ_TRACK) begin
	                dbg_load_blocks <= dbg_load_blocks + {2'b0,read_burst_blocks};
	            end
	            if (sd_ack) begin
	                if (sd_buff_wr) begin // Reading from SD -> RAM
	                     ...
	                     if (state == S_SCAN_WOZ && !scan_skip_discard &&
	                         ...
	                         if (hdr_pos < 4'd12) begin
	                             ...
	                             case (hdr_pos)
                                     0: if (b != "W") header_ok <= 0;
                                     1: if (b != "O") header_ok <= 0;
                                     2: if (b != "Z") header_ok <= 0;
                                     3: if (b != "1" && b != "2") header_ok <= 0;
                                     4: if (b != 8'hff) header_ok <= 0;
                                     5,7: if (b != 8'h0a) header_ok <= 0;
                                     6: if (b != 8'h0d) header_ok <= 0;
                                     default: ;
                                 endcase
                         ...
                         if (is_woz_v1) begin
                             ...
                             v1_raw_addr = {dma_block[6:0], sd_buff_addr[8:0]};
                             ...
                             track_load_addr <= {dma_block[6:0], sd_buff_addr[8:0]};
                             track_load_data <= sd_buff_dout;
                             track_load_we <= 1;
                             if (pending_is_flux) begin
