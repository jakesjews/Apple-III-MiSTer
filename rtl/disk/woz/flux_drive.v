...
module flux_drive (
    ...
    output wire [8:0]  BIT_TIMER_OUT,   // Current bit cell timer value (for IWM sync)
    ...
    reg [8:0]   bit_timer;              // Countdown for bit cell timing
    reg [8:0]   bit_half_timer;         // Mid-cell timing for flux pulses
    reg [9:0]   bit_cell_frac;          // Fractional bit cell accumulator (3.5")
    reg [9:0]   bit_half_frac;          // Fractional half-cell accumulator (3.5")
    reg [8:0]   bit_cell_cycles_reg;    // Per-bit-cell length (with fractional add)
...
`endif
    ...
    // Honor the image timing in 125 ns units. The P6 sequencer has its own
    // motherboard Q3 clock, independent of this media bit rate.
    wire [8:0]  cell525_base;
    wire [9:0]  cell525_step;
    wire [8:0]  half525_base;
    ...
    wire [8:0]  bit_cell_base = IS_35_INCH ? BIT_CELL_35 : cell525_base;
...
`endif
    wire [8:0]  bit_half_base = IS_35_INCH ? (BIT_CELL_35 >> 1) : half525_base;
    ...
    // 5.25-inch timing also retains the fractional part of its cell period.
    wire [9:0]  bit_cell_step = IS_35_INCH ? 10'd636 : cell525_step;
    wire [9:0]  bit_half_step = IS_35_INCH ? 10'd318 : half525_step;
    wire [8:0]  bit_cell_cycles = bit_cell_cycles_reg;
...
`endif
    ...
    always @(posedge CLK_14M or posedge RESET) begin
        if (RESET) begin
            bit_position <= 17'd0;
            bit_timer <= 6'd56;  // Start at full bit cell time
            bit_half_timer <= 6'd28;
            bit_cell_cycles_reg <= 6'd56;
            bit_cell_frac <= 10'd0;
            bit_half_frac <= 10'd0;
