...
module apple3_acia #(
	...
	(* altera_attribute = "-name SYNCHRONIZER_IDENTIFICATION FORCED_IF_ASYNCHRONOUS" *) logic [3:0] serial_meta, serial_sync;
	always_ff @(posedge clk) begin
		if (reset) begin
			serial_meta <= 4'b1111;
			serial_sync <= 4'b1111;
	...
	gen_uart_mos_6551 uart (
		.clk(clk), .reset(reset), .clk_en(baud_enable),
		// RxC is grounded on the Apple /// motherboard (sheet 8).
		.rx_clk_en(1'b0),
		.cs(read_strobe || write_strobe), .rnw(!write_strobe), .rs(addr),
		.din(data_in), .dout(data_out), .irq_n(irq_n),
		.rx(serial_sync[0]), .cts_n(serial_sync[1]),
		.dsr_n(serial_sync[2]), .dcd_n(serial_sync[3]),
		.tx(tx), .rts_n(rts_n), .dtr_n(dtr_n)
	);
endmodule
