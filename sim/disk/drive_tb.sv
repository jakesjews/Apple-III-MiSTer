...
module drive_tb;
    ...
    flux_drive drive (
        ...
        .PHASES(phases), .IMMEDIATE_PHASES(phases), .LATCHED_SENSE_REG(3'd0),
        .IWM_MODE(5'd0), .MOTOR_ON(active), .SW_MOTOR_ON(active),
        ...
        .BIT_POSITION(bit_position), .BIT_TIMER_OUT(), .TRACK_BIT_COUNT(bit_count),
    ...
    integer cycles=0,last_flux=-1,pulses=0,short_cells=0,long_cells=0,writes=0,last_cell=-1,cells=0;
    integer previous_position=0,wraps=0,last_wrap=0;
    reg check_clock=1;
    always @(negedge clk) if(!engine_reset) begin
        cycles=cycles+1;
        if(flux && valid) begin
            if(check_clock && pulses>10 && (cycles-last_flux<56 || cycles-last_flux>59))
                $fatal(1,"unexpected flux interval %0d",cycles-last_flux);
            last_flux=cycles;pulses=pulses+1;
        end
        if(!valid) pulses=0;
        if(bit_position!=previous_position && !write_mode) begin
            ...
            last_cell=cycles;cells=cells+1;
        end
        if(previous_position>50000 && bit_position==0) begin
            ...
            last_wrap=cycles;wraps=wraps+1;
        end
    ...
    task measure_timing(input [7:0] value,input integer expected);
        ...
        begin
            ...
            start=cycles;count=0;position=bit_position;
            while(count<1000) begin
                @(negedge clk);
                if(bit_position!=position) begin count++;position=bit_position;end
            end
    ...
    endtask
    initial begin
        repeat(5) @(negedge clk);engine_reset=0;
        wait(wraps==2);
        if(short_cells==0 || long_cells<25000) $fatal(1,"fractional clock overflow: short=%0d long=%0d",short_cells,long_cells);
        // Missing cached data must suppress pulses without stopping rotation.
        @(negedge clk);valid=0;
        repeat(1000) begin @(negedge clk);if(flux) $fatal(1,"stale flux while loading");end
        valid=1;
        check_clock=0;
        measure_timing(28,50113);
        measure_timing(255,456392);
        // Protection must block the read-modify-write path.
        write_mode=1;repeat(5) @(negedge clk);
        write_strobe=1;@(negedge clk);write_strobe=0;
        repeat(5) @(negedge clk);if(writes) $fatal(1,"protected write");
        write_protect=0;write_bit=0;
        write_strobe=1;@(negedge clk);write_strobe=0;
        ...
        $display("PASS drive: WOZ 4us/3.5us/31.875us cells, fractional timing, cache gating, write protection and bit writes");
        $finish;
    end
