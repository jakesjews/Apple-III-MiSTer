...
module woz_image_tb;
	...
	task wait_track;
		...
		begin
			n = 0;
			while (!(ready && track_data_valid) && n < 300000) begin
				@(posedge clk);
				n = n + 1;
			end
	...
	initial begin
		...
		file = $fopen(path, "rb");
		size = $fread(media, file);
		$fclose(file);
		img_size   = size;
		version    = media[3] - "0";
		allocation = version == 2 ? media[258] + 256 * media[259] : 13;
		...
		if (version == 2) begin
			...
			active = 0;
			n      = 0;
			while ((writes < allocation || busy || phase != 0) && n < 100000) begin
				@(posedge clk);
				n = n + 1;
			end
			...
			for (integer i = 0; i < size; i++) begin
				...
				end else if (media[i] !== original[i])
					$fatal(1, "unrelated byte changed %0d %02x != %02x", i, media[i], original[i]);
			end
		end
		active   = 1;
		track_id = 4;
		...
		$display("PASS WOZ%0d: block delivery, exact track data, quarter tracks, reset, rejection, writes=%0d",
				 version, writes);
		$finish;
	end
	initial begin
