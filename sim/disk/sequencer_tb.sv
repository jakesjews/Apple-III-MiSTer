...
module sequencer_tb;
    ...
    initial begin
        ...
        for(integer i=0;i<50000;i++) begin
            ...
            q6=random_state[29];q7=random_state[28];write_protect=random_state[27];
            data_in=random_state[7:0];
            q3=1;
            repeat(3) @(negedge clk);
            flux=random_state[31];
            @(negedge clk); q3=0;flux=0;
        ...
        for(integer value=0;value<256;value++) begin : write_test
            ...
            @(negedge clk);reset=1;q3=0;flux=0;
            repeat(3) @(negedge clk);reset=0;q7=1;q6=1;data_in=value;
            // Stop immediately after the sequencer's parallel-load operation.
            watchdog=0;
            while(dut.state != 4'b0011 && watchdog<32) begin
                q3=1;repeat(4) @(negedge clk);
                q3=0;repeat(3) @(negedge clk);
                watchdog++;
            end
            if(data_out!==value[7:0]) $fatal(1,"parallel write load");
            q6=0;emitted=0;count=0;watchdog=0;
            while(count<8 && watchdog<80) begin
                q3=1;repeat(4) @(negedge clk);
                q3=0;@(posedge clk);#1;
                if(write_strobe) begin emitted={emitted[6:0],write_bit};count++;end
                repeat(3) @(negedge clk);watchdog++;
            end
            if(count!=8 || emitted!==value[7:0])
                $fatal(1,"write stream %02x != %02x, count=%0d",emitted,value,count);
        end
        $display("PASS P6: all 256 truth-table entries and 50000 sequencer cycles against original PROM, all 256 written bytes");
        $finish;
    end
endmodule
