...
module mmu_tb;
	...
	task automatic expect_decode(
		input rram, input wram, input rrom, input io, input vd, input ve
	);
	begin
		#1; checks = checks + 1;
		if ({ram_read,ram_write_allowed,rom_read,io_select,via_d_select,via_e_select} !==
		    {rram,wram,rrom,io,vd,ve}) begin
			$display("FAIL decode @%04x got %b%b%b%b%b%b expected %b%b%b%b%b%b",
				cpu_addr, ram_read,ram_write_allowed,rom_read,io_select,via_d_select,via_e_select,
				rram,wram,rrom,io,vd,ve);
			$fatal(1);
		end
	end
	endtask
