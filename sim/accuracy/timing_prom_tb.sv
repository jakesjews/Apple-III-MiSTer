...
module timing_prom_tb;
  ...
  initial begin
    ...
    wait(h_state==10 && state_dot==6); #1;
    if(cpu_enable !== timing_prom['h0da][1]) begin
      failures++;
      $display("FAIL: fast non-RAM display A-slot: PROM PHASEN=%b, RTL CPU enable=%b",
               timing_prom['h0da][1],cpu_enable);
    end
    ...
    for(integer clocks=0;clocks<912*262;clocks++) begin
      ...
      if(state_dot==6 && h_state<64) begin
        ...
        for(integer config_bits=0;config_bits<8;config_bits++) begin
          ram_cycle=config_bits[0]; screen_enable=config_bits[1]; slow_mode=config_bits[2];
          ...
          if(cpu_enable !== timing_prom[phase_address][1]) begin
            if(phase_errors<5) $display("PHASEN mismatch v=%0d h=%0d PROM[%x] config=%b",v_count,h_state,phase_address,config_bits);
            phase_errors++;
          end
        end
        ram_cycle=1; screen_enable=1; slow_mode=0; #0.01;
        if(refresh_slot !== scan_prom[scan_address][4]) begin
          if(mismatches<8) $display("  refresh mismatch V=%0d H=%0d PROM[%03x]=%b RTL=%b",
            v_count,h_state,scan_address,scan_prom[scan_address][4],refresh_slot);
          mismatches++;
          if(scan_prom[scan_address][4]) missing_refresh++; else extra_refresh++;
          if(v_count>=192) blank_mismatches++; else visible_mismatches++;
        end
      end
    end
    $display("refresh PROM: %0d/%0d mismatches (visible=%0d, blank=%0d)",
             mismatches,checked,visible_mismatches,blank_mismatches);
