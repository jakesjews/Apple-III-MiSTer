...
module video_accuracy_tb;
  ...
  initial begin
    ...
    for(integer dot=0;dot<28;dot++) begin
      h_count=dot; h_state=dot/14; state_dot=dot%14; #1;
      if(dut.colour_index !== pixels[(dot/4)*4 +: 4]) begin
        mismatches++;
        $display("  140-mode dot %0d: expected colour %x, got %x",dot,
                 pixels[(dot/4)*4 +: 4],dut.colour_index);
      end
    end
    ...
    for(integer mode=8;mode<12;mode++) begin
      h_count=600; v_count=0; video_mode=mode; smooth_enable=0; #1;
      before_scroll={dut.request_lane,ram_addr};
      smooth_enable=1; smooth_offset=1; #1;
      check({dut.request_lane,ram_addr}!=before_scroll,$sformatf("graphics mode %x scroll changes fetched row",mode));
    end
        ...
        for(integer offset=0;offset<8;offset++)
          for(integer plane=0;plane<2;plane++)
            for(integer boundary=0;boundary<2;boundary++) begin
              video_mode=mode; v_count=(y==0)?261:y-1;
              smooth_offset=offset; h_count=600+40*plane+39*boundary; #1;
              ...
              if(ram_addr!==expected_word[17:0] || dut.request_lane!==((physical>>11)&1)) begin
                if(address_errors<5) $display("graphics address mismatch mode=%x y=%0d scroll=%0d physical=%x word=%x lane=%b",
                  mode,y,offset,physical,ram_addr,dut.request_lane);
                address_errors++;
              end
            end
    ...
    smooth_enable=0; native_mode=0; video_mode=4'h9;
    h_state=0; state_dot=0; h_count=0; v_count=0;
    dut.line_buffer_low[0]=8'hc1;
    dut.line_buffer_high[0]=8'h00;
    dut.character_ram[{7'h41,3'd0}]=8'h01; #1;
    ...
    h_count=600; v_count=0; #1;
    check(ram_addr==18'h3c400 && !dut.request_lane,"Apple II TEXT fetch overrides HIRES");
    h_count=0; repeat(3) @(negedge clk);
    dut.line_buffer_low[0]=8'h21; dut.line_buffer_high[0]=8'h43;
